...
function convertToNumber($book = NULL, $database = NULL, $dbname='') {
  if (!$database) { die('you forgot to specify the database in your bible_to_sql call.'); }

  $query = "SELECT B from ".((isset($dbname) && !empty($dbname)) ? trim($dbname).'.' : '')."key_abbreviations_english WHERE A=?";
  $stmt = $database->stmt_init();
  //$stmt= $mysqli->prepare($query) or die( $mysqli->error);
  $stmt->prepare($query) or die( $mysqli->error);
...
function convertToBook($number = NULL, $database = NULL, $dbname='') {
  if (!$database) { die('you forgot to specify the database in your bible_to_sql call.'); }
  $query = "SELECT n from ".((isset($dbname) && !empty($dbname)) ? trim($dbname).'.' : '')."key_english WHERE b=?";
...
function getMainAbbr($number = NULL, $database = NULL, $dbname=''){
  if (!$database) { die('you forgot to specify the database in your bible_to_sql call.'); }
  $query = "SELECT a FROM ".((isset($dbname) && !empty($dbname)) ? trim($dbname).'.' : '')."`key_abbreviations_english` WHERE b=? ORDER BY p DESC, id ASC LIMIT 1";
  $stmt = $database->stmt_init();
  $stmt->prepare($query);
  $stmt->bind_param("s", $number);
  $stmt->execute();
...
class bible_to_sql {
	...
	public function __construct($string = NULL, $range = FALSE, $database = NULL) {
		...
		if (!isset($separatedVerse[1]) || !$separatedVerse[1]) {
			$range = TRUE;
		}

		//set range
		$this->range = $range;
		$splitarr = (isset($separatedVerse[1]) ? $separatedVerse[1] : '');
		if ($range) {
			$anotherSplit = explode("-",$splitarr);
			$this->verse = $this->addZeros($anotherSplit[0],3);
				if (isset($anotherSplit[1]) && $anotherSplit[1]) {
